    ...
    this.listView = new SelectList({
      ...
      didConfirmSelection: ({
        ...
        atom.workspace.getActiveTextEditor().save().then(() => {
          exec(toExec, (a, b, c) => { console.log(a, b, c); });
        })
      },
  ...
  bigBoy (selectedText, filePath, items) {
    ...
    exec(hoogleMore, (err, stdout, stderr) => {
      if (err && items.length == 0) {
        atom.notifications.addWarning("Failed to get results from global hoogle backup query. Make one if you want this to work");
        return;
      }
